...
- external credentials assigned by purpose to protected environments restricted to `main`. GitHub
  ...
  maintainer and Dependabot events. The Actions twin remains in the `dependabot-automation`
  environment restricted to `main` and is referenced only by the `pull_request_target` path,
  which GitHub loads from the trusted base branch. The Dependabot twin serves only `pull_request`
  events sent by Dependabot because Dependabot secrets cannot be environment-scoped. Neither path
  checks out or executes pull-request code, and both reject forks. Other workflows triggered by
  fork pull requests and by Dependabot receive no user-managed Actions secret;
...
Queue admission is human for human-authored pull requests. Canonical same-repository pull requests authored by `dependabot[bot]` are the deliberate exception: a repository-local workflow may use a short-lived, repository-scoped GitHub App token to enable GitHub native auto-merge for the exact event head. Dependabot-emitted `opened` and `synchronize` events use only `pull_request`; code-neutral `ready_for_review` and `reopened` transitions use only `pull_request_target`, whose workflow GitHub loads from the trusted base branch, and require the sole operator `lcv-leo` by immutable account ID and actor login. Before admission, the workflow reads the exact head through GitHub's commit API and requires the Dependabot account, GitHub's `web-flow` committer, and a valid GitHub signature as defense in depth. The authoritative controls remain the exact sender allowlists, the active Enterprise ruleset that restricts updates to `dependabot/**` branches to the approved Apps, and `--match-head-commit`. The workflow executes no pull-request code and neither updates branches nor repairs lockfiles. The native merge queue still runs every required check on its synthetic `merge_group` revision; the automation cannot bypass rules, merge directly, or process forks, drafts, conflicting heads, or any other author.

Dependabot auto-merge is intentionally independent per repository and uses GitHub's native auto-merge and merge queue rather than an organization-wide controller or scheduler. The same private key is stored under the same name in the repository's Dependabot secret store and in the protected `dependabot-automation` Actions environment: GitHub supplies the Dependabot copy to Dependabot-emitted `pull_request` runs, and the environment copy to maintainer-emitted `pull_request_target` runs whose base ref is `main`. It is exchanged at runtime for a short-lived token limited to pull-request, merge-queue, and repository-content merge operations, as required by GitHub's native merge API. The token is exposed only to the provenance and `gh pr merge` steps; the workflow checks out and executes no pull-request code. Repository-local workflows may secure and publish only this repository's own institutional surfaces. Enterprise or organization rules, settings, applications, and secrets require separate explicit operator consent before any change.
