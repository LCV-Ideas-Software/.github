...
Queue admission is human for human-authored pull requests. Canonical same-repository pull requests authored by `dependabot[bot]` are the deliberate exception: a repository-local workflow may use a short-lived, repository-scoped GitHub App token to enable GitHub native auto-merge for the exact event head. Dependabot-emitted events use `pull_request`; code-neutral `ready_for_review` and `reopened` transitions emitted by a maintainer use `pull_request_target`, whose workflow GitHub loads from the trusted base branch. The action-aware condition makes the duplicate event in the other context skip before credentials are consumed. Before admission, the workflow reads the exact head through GitHub's commit API and requires the Dependabot account, GitHub's `web-flow` committer, and a valid GitHub signature. This complements the active Enterprise ruleset that restricts updates to `dependabot/**` branches to the approved Apps. The workflow executes no pull-request code and neither updates branches nor repairs lockfiles. The native merge queue still runs every required check on its synthetic `merge_group` revision; the automation cannot bypass rules, merge directly, or process forks, drafts, conflicting heads, or any other author.
