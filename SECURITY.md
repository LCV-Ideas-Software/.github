...
- external credentials assigned by purpose to protected environments restricted to `main`. GitHub
  ...
  workflow explicitly references it. The repository-local Dependabot admission App has encrypted
  twins in the Actions and Dependabot stores because GitHub selects a different store for
  operator and Dependabot events. The Actions twin remains in the `dependabot-automation`
  environment restricted to `main` and is referenced only by the trusted `workflow_dispatch`
  fallback. The Dependabot twin serves only `pull_request` events sent by Dependabot because
  Dependabot secrets cannot be environment-scoped. Neither path checks out repository content,
  and both reject forks. Other workflows triggered by fork pull requests and by Dependabot receive
  no user-managed Actions secret;
...
Queue admission is human for human-authored pull requests. Canonical same-repository pull requests authored by `dependabot[bot]` are the deliberate exception: a repository-local workflow may use a short-lived, repository-scoped GitHub App token to enable GitHub native auto-merge for the exact event head. Dependabot-emitted `opened` and `synchronize` events use the GitHub- and Zizmor-documented `pull_request` plus dedicated Dependabot-secret pattern. A rare pull request that is manually made ready or reopened requires either a new Dependabot synchronization or the explicit `workflow_dispatch` fallback from `main`; that fallback accepts a pull-request number only from the sole operator `lcv-leo`, identified by immutable account ID, on the first run attempt. Before admission, both paths read the exact head through GitHub's APIs and require a same-repository, non-draft, open `main` pull request, a `dependabot/**` branch, the Dependabot account, GitHub's `web-flow` committer, and a valid GitHub signature as defense in depth. The authoritative controls remain the exact sender/operator allowlists, the active Enterprise ruleset that restricts updates to `dependabot/**` branches to the approved Apps, and `--match-head-commit`. The workflow checks out no repository content and neither updates branches nor repairs lockfiles. The native merge queue still runs every required check on its synthetic `merge_group` revision; the automation cannot bypass rules, merge directly, or process forks, drafts, conflicting heads, or any other author.

Dependabot auto-merge is intentionally independent per repository and uses GitHub's native auto-merge and merge queue rather than an organization-wide controller or scheduler. The same private key is stored under the same name in the repository's Dependabot secret store and in the protected `dependabot-automation` Actions environment: GitHub supplies the Dependabot copy to Dependabot-emitted `pull_request` runs, and the environment copy only to an operator-triggered `workflow_dispatch` run on `main`. It is exchanged at runtime for a short-lived token limited to pull-request, merge-queue, and repository-content merge operations, as required by GitHub's native merge API. The token is exposed only to the provenance and `gh pr merge` steps; the workflow checks out no repository content. Repository-local workflows may secure and publish only this repository's own institutional surfaces. Enterprise or organization rules, settings, applications, and secrets require separate explicit operator consent before any change.
