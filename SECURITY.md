...
Queue admission is human for human-authored pull requests. Canonical same-repository pull requests authored by `dependabot[bot]` are the deliberate exception: a repository-local workflow may use a short-lived, repository-scoped GitHub App token to enable GitHub native auto-merge for the exact event head. It accepts only events emitted by Dependabot's immutable App identity, executes no pull-request code, and neither updates branches nor repairs lockfiles. The native merge queue still runs every required check on its synthetic `merge_group` revision; the automation cannot bypass rules, merge directly, or process forks, drafts, conflicting heads, or any other author.

Dependabot auto-merge is intentionally independent per repository and uses GitHub's native auto-merge and merge queue rather than an organization-wide controller or scheduler. Its private key is stored only as a repository Dependabot secret and is exchanged at runtime for a short-lived token limited to pull-request, merge-queue, and repository-content merge operations, as required by GitHub's native merge API. The token is exposed only to the `gh pr merge` step; the workflow checks out and executes no pull-request code. Repository-local workflows may secure and publish only this repository's own institutional surfaces. Enterprise or organization rules, settings, applications, and secrets require separate explicit operator consent before any change.
