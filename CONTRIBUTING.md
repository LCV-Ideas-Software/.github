...
- Authorized exception: `.github/workflows/scorecard.yml` runs under least privilege (`security-events: write`, `contents: read`, `actions: read`). The operator authorized this on 2026-08-10 so that workflow requests only what it uses; every permission there carries an inline comment, as zizmor's `undocumented-permissions` audit requires. This is a privilege decision, not a merge-queue fix - the queue was blocked by a review-parser defect and by stale generated types, both addressed separately. Note that the baseline does constrain OpenSSF: its publication API rejects organization-mandated `write-all`, which is why `publish_results` is disabled. Any further exception needs the same explicit authorization and an entry here.
