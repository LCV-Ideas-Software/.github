...
- Preserve Dependabot checks every day at 05h (UTC−03:00), including weekends, grouped minor and patch version updates, separate security-update groups, automatic rebasing, and GitHub's post-merge branch deletion. Official Actions under `actions/*` and `github/*` are evaluated immediately; third-party GitHub Actions and ordinary version updates in every other ecosystem observe a seven-day cooldown. Security updates are not delayed by that cooldown.
