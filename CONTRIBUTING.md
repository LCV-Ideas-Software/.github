...
- Set workflow-level permissions to `{}` and grant each job only the `GITHUB_TOKEN` capabilities it demonstrably needs. Keep privileged external credentials in protected environments and out of pull-request jobs. The repository-local Dependabot admission key has two encrypted copies because GitHub selects secrets by event origin: the Actions copy is restricted to `main` by the `dependabot-automation` environment and may be referenced only through the trusted-base `pull_request_target` path; the Dependabot copy serves only Dependabot-emitted `pull_request` runs because Dependabot secrets cannot be environment-scoped. Neither path checks out pull-request code.
...
- Dependabot prepares and rebases its pull requests automatically. Each repository independently uses a repository-local workflow to enable GitHub native auto-merge for a canonical same-repository Dependabot pull request on the exact event head SHA. Dependabot-emitted `opened` and `synchronize` events run exclusively through `pull_request`; only the sole operator `lcv-leo` (immutable account ID `268063598`) may emit the code-neutral `ready_for_review` and `reopened` transitions through `pull_request_target`. Every path verifies the exact head through GitHub's commit API as a GitHub-signed commit associated with the Dependabot account before admission. This is defense in depth over the authoritative sender allowlists, the active Enterprise branch-update ruleset, and `--match-head-commit`. The workflow executes no pull-request code, uses a short-lived repository-scoped GitHub App token, and cannot bypass the merge queue or required checks. It does not update branches or repair lockfiles.
