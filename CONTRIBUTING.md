...
- Set workflow-level permissions to `{}` and grant each job only the `GITHUB_TOKEN` capabilities it demonstrably needs. The repository-local Dependabot admission key has two encrypted copies because GitHub selects secrets by event origin: the Actions copy is restricted to `main` by the `dependabot-automation` environment and may be referenced only by the explicit `workflow_dispatch` fallback; the Dependabot copy serves only Dependabot-emitted `pull_request` runs because Dependabot secrets cannot be environment-scoped. Neither path checks out repository content.
...
- Dependabot prepares and rebases its pull requests automatically. Each repository independently uses a repository-local workflow to enable GitHub native auto-merge for a canonical same-repository Dependabot pull request on the exact event head SHA. Dependabot-emitted `opened` and `synchronize` events run exclusively through the documented `pull_request` path. A rare manual ready/reopen transition requires another Dependabot synchronization or an explicit `workflow_dispatch` from `main`; only the sole operator `lcv-leo` (immutable account ID `268063598`) can invoke that fallback. Every path resolves the live pull request and verifies the exact head through GitHub's APIs as a GitHub-signed commit associated with the Dependabot account before admission. This is defense in depth over the authoritative sender/operator allowlists, the active Enterprise branch-update ruleset, and `--match-head-commit`. The workflow checks out no repository content, uses a short-lived repository-scoped GitHub App token, and cannot bypass the merge queue or required checks. It does not update branches or repair lockfiles.
