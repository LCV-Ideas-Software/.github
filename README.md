...
- Dependabot checks GitHub Actions, npm, Deno, and the pinned Zizmor container daily and automatically rebases its pull requests. GitHub Actions updates are evaluated immediately; the other ecosystems apply a seven-day cooldown to ordinary version updates. The cooldown does not apply to security updates.
...
- The seven repository-local required contexts share one YAML-parsed canonical contract. Dependency Review and both CodeQL matrix jobs execute it independently on `pull_request` and `merge_group`, so a one-line skip, failure mask, trigger drift, critical-path change, or Zizmor suppression fails in a different required context.
- The reusable Zizmor workflow accepts reviewed consumer configuration and inline ignores only when both the base and candidate snapshots match the repository/path/mode/blob-SHA inventory in [`.github/zizmor/policy-baselines.v1.json`](./.github/zizmor/policy-baselines.v1.json). The candidate cannot provide or widen this allowlist; changing a protected blob requires a separately reviewed central component release.
