...
- The static organization site lives in [`site/`](./site/) and is deployed from that directory as the root of the Cloudflare Pages project `org-site`, whose canonical public domain is <https://www.lcv.dev>. The sponsor landing page lives in [`site/sponsor/`](./site/sponsor/) and calls the dedicated `sponsor-motor` Worker at `https://sponsor-motor.lcv.app.br`.
