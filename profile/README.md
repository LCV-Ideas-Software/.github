...
- **Direct-to-`main` baseline.** `main` is the default and deploy branch. Direct signed, fast-forward pushes are allowed. Default branches cannot be deleted or force-pushed; tags require signed commits and cannot be deleted or updated. PRs and auto-merge may be used, but are not mandatory, and there is no universal `preview → main` promotion flow.
