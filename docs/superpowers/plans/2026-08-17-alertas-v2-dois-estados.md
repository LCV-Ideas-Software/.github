...
- **Matriz de escritas (testada por mutação na Tarefa 2; emendada na revisão da implementação):** o carimbo (`attempts`, `updated_ms`, `next_due_ms`, via o mesmo CAS) pertence ao **papel de agendador** — o CRON o executa a cada passe (e apaga `sent` velho), e o INGRESS o executa **uma vez, no aceite**, para a tentativa 1 ("publica só quem carimba" vale inclusive para ele; a redação original, "INSERT e nada depois", caiu com essa emenda); CONSUMIDOR escreve `state='sent'` + `slack_message_ts` no sucesso e `last_error` no fracasso — **nunca** colunas de agendamento; `created_ms` nunca é escrito depois do INSERT.
