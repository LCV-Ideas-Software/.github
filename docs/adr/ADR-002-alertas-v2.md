...
| 3 | Segredo compartilhado do `/status`, dos dois lados | **feito e rotacionado em 17/08** — Secrets Store `github-slack-alerts-status-secret` (id `815c2dc2…`, valor novo) + `ALERTS_STATUS_SECRET` **no environment `alerts-watchdog`** (dedicado, sem regras de proteção — exigência do zizmor; o repo-level foi removido) + binding na fonte; **deploy executado em 17/08** (run #381 verde; `/alerts/status` respondendo com o segredo novo) | `gh secret list --env alerts-watchdog` e o `Env` gerado |
| 4 | Fila de descarte removida (decisão 8) | **fonte: feito em 17/08** — consumidor de alertas sem `dead_letter_queue`, `max_retries: 0` (mínimo não documentado; o primeiro deploy valida, fallback registrado = 1), DLQ fora dos consumers, docs atualizados; teste de contrato da configuração prende tudo (RED antes). O código mantém a guarda defensiva; o **recurso** `github-slack-alerts-dlq` foi **apagado da Cloudflare em 17/08**, após o deploy verde — item completo | teste `config.test.ts` + ausência nos docs |
