...
const CONFIG = JSON.stringify({
  ...
  imports: {
    "npm:esbuild@0.24.2": "npm:esbuild@0.28.2",
...
const LOCK = JSON.stringify({
  specifiers: { "npm:esbuild@0.28.2": "0.28.2" },
  npm: {
    "esbuild@0.28.2": {
      integrity:
...
Deno.test("validates the exact local hook, override, and clean lockfile", () => {
  ...
  assertThrows(
    () =>
      verifyLocalPins(
        HOOKS,
        CONFIG.replace("npm:esbuild@0.28.2", "npm:esbuild@^0.28.2"),
        LOCK,
      ),
    "exact reviewed import-map surface",
  );
    ...
    () =>
      verifyLocalPins(
        ...
        LOCK.replace(
          ...
          '"npm:esbuild@0.24.2":"0.24.2","npm:esbuild@0.28.2":"0.28.2"',
        ),
      ),
    "unexpected esbuild specifier set",
  );
  const changedPackageIntegrity = JSON.parse(LOCK);
  changedPackageIntegrity.npm["esbuild@0.28.2"].integrity = "sha512-unreviewed";
  assertThrows(
    () =>
      verifyLocalPins(
        HOOKS,
  ...
  delete missingOpenHarmonyPlatform.npm["@esbuild/openharmony-arm64@0.28.2"];
  assertThrows(
    () =>
      verifyLocalPins(
        HOOKS,
  ...
  changedPlatformIntegrity.npm["@esbuild/linux-x64@0.28.2"].integrity =
    "sha512-unreviewed";
  assertThrows(
    () =>
      verifyLocalPins(
