...
Deno.test(
  "configuration rejects absent, malformed, duplicate, or padded identifiers",
  () => {
    for (const name of Object.keys(environment)) {
      const candidate = { ...environment } as Record<
        string,
        string | undefined
      >;
...
Deno.test(
  "fails closed on Slack errors, pagination, extras, and duplicates",
  () => {
    const configuration = readTriggerInventoryConfiguration(environment);
    for (
      const candidate of [
...
Deno.test(
  "fails closed on type, app, callback, name, URL, or input drift",
  () => {
    const configuration = readTriggerInventoryConfiguration(environment);
...
Deno.test(
  "CLI mode never renders bearer URLs, payloads, or parser details",
  async () => {
