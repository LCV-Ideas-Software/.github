...
Deno.test("authenticates the pre-send boundary and preserves the validated message", async () => {
  let calls = 0;
  const result = await reportRelayProgress(
    ...
    SECRET,
    {
      ...
      fetchImpl: async (url, init) => {
        calls += 1;
        assert(
          url === "https://github-slack-alerts.lcv.workers.dev/slack/progress",
        ...
        assert(body.message_ts === "", "pre-send receipt carried a message ts");
        assert(
...
Deno.test("retries an idempotent delivered receipt once without posting another message", async () => {
  const bodies: string[] = [];
  const result = await reportRelayProgress(
...
Deno.test("fails closed before network on malformed progress or a short secret", async () => {
  let calls = 0;
  const fetchImpl = () => {
    calls += 1;
  ...
  assert(
    !(await reportRelayProgress(malformed, SECRET, {
      now: () => NOW,
      fetchImpl,
    })).ok,
